...
<div style="margin-left: 120px; margin-right: 120px;">
    ...
    <div class="content-wrapper">
        <form action="">
            <div class="input-wrapper">
                ...
                <input style="width: 80%;" class="text-input" type="text" value="<?php echo $user["username"] ?>">
            </div>
            <div class="input-wrapper">
                <p class="input-attribute">Email</p>
                <input style="width: 80%;" class="text-input" type="text" value="<?php echo $user["email"] ?>">
            </div>
            <div class="input-wrapper">
                <p class="input-attribute">Date of Birth</p>
                <input style="width: 80%;" class="text-input" type="text" value="<?php echo $user["dob"] ?>">
            </div>
            <div class="input-wrapper">
                <p class="input-attribute">Nationality</p>
                <input style="width: 80%;" class="text-input" type="text" value="<?php echo $user["nationality"] ?>">
            </div>
            <div class="input-wrapper">
    ...
    <?php
    // follow
    $stmt = $conn->prepare("SELECT * FROM users, follow, streamers WHERE streamers.streamerID = ? AND users.userID = follow.userID AND follow.streamerID = streamers.streamerID");
    $stmt->execute(array($_SESSION["ID"]));
    $followCount = count($stmt->fetchAll(PDO::FETCH_ASSOC));
    // subscribe
    $stmt = $conn->prepare("SELECT * FROM users, subscribe, streamers WHERE streamers.streamerID = ? AND users.userID = subscribe.userID AND subscribe.streamerID = streamers.streamerID");
    $stmt->execute(array($_SESSION["ID"]));
    $subCount = count($stmt->fetchAll(PDO::FETCH_ASSOC));
    // donate
    $stmt = $conn->prepare("SELECT users.username AS donateName, donation_amount, donation_message FROM users, donate, streamers WHERE streamers.streamerID = ? AND users.userID = donate.userID AND donate.streamerID = streamers.streamerID");
    $stmt->execute(array($_SESSION["ID"]));
    ...
    <div id="streamer-stats">
        <div class="content-wrapper">
            ...
            <div class="statistics-wrapper">
                ...
                <div>Total donation earned: $<?php echo $donateSum ?>
                </div>
            </div>
        </div>
        <!-- Enable this only when streamer is active -->
